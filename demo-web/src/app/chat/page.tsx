        ...
        <div className="space-y-2 w-full">
            {pd.retrievedDocs && pd.retrievedDocs.length > 0 && (
                <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
                    ...
                    {showR && <div className="px-3 pb-3 space-y-1.5 border-t border-slate-100 pt-2 max-h-72 overflow-y-auto">{pd.retrievedDocs.map((d, i) => <DocCard key={d.id} doc={d} rank={i + 1} />)}</div>}
                </div>
            )}

            {pd.rerankedDocs && pd.rerankedDocs.length > 0 && <div className="flex justify-center"><ArrowRight className="w-4 h-4 text-slate-200" /></div>}
            ...
            {pd.rerankedDocs && pd.rerankedDocs.length > 0 && (
                <div className="rounded-xl border border-indigo-200 bg-white overflow-hidden">
                    ...
                    {showRR && <div className="px-3 pb-3 space-y-1.5 border-t border-indigo-100 pt-2 max-h-72 overflow-y-auto">{pd.rerankedDocs.map((d, i) => <DocCard key={d.id} doc={d} rank={i + 1} isReranked />)}</div>}
                </div>
            )}
